...
const JupiterSwap = () => {
  ...
  const balancesLoaded = React.useRef(false);
  const [settings, setSettings] = useState(() => {
    const saved = localStorage.getItem('guvSettings');
    return saved ? JSON.parse(saved) : { slippageTolerance: 0.5, confirmTransactions: true };
  });
  ...
  const fetchWalletBalances = useCallback(async (force = false) => {
    ...
    if (balancesLoaded.current && !force && balancesLoaded.current === true && Object.keys(walletBalances).length > 0) {
      return;
    }
  ...
  // Fetch balances when wallet connects or tab becomes active
  useEffect(() => {
  ...
  const fetchQuote = useCallback(async () => {
    ...
    // Fetch balances if not loaded yet
    if (!balancesLoaded.current && connected && publicKey) {
    ...
    try {
      ...
      // Only check balance for SOL (keep for fees)
      if (fromToken === 'SOL' && inputAmount > balance) {
      ...
      const slippageBps = Math.round(settings.slippageTolerance * 100);

      const url = `https://api.jup.ag/ultra/v1/order?inputMint=${fromTokenData.mint}&outputMint=${toTokenData.mint}&amount=${amount}&slippageBps=${slippageBps}&prioritizationFeeLamports=0`;
      ...
      if (!response.ok) {
        ...
        if (errorText.includes('insufficient') || errorText.includes('lamports')) {
          throw new Error('Insufficient SOL for fees. Need more SOL.');
        }
        throw new Error('Quote failed. Try again.');
      }
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      if (!orderResponse.ok) {
        ...
        setSwapError('Quote expired. Refresh to continue.');
        setQuote(null);
        return;
      }
      ...
      try {
        ...
        if (!executeResponse.ok) {
          ...
          if (errorText.includes('expired') || errorText.includes('Expired')) {
            setSwapError('Quote expired. Refreshing...');
            setQuote(null);
            await fetchQuote();
            return;
          }
          ...
          if (signature) {
            setLastSwapTxId(signature);
            setSwapSuccess(`Swapped ${fromAmount} ${fromToken} for ${toToken}! Tx submitted.`);
            setFromAmount('');
            setQuote(null);
            fetchWalletBalances();
            return;
          ...
          if (executeResult.status === 'Success' || executeResult.status === 'success' || executeResult.success || executeResult.signature) {
            const outAmount = quote ? (parseFloat(quote.outAmount) / Math.pow(10, getTokenBySymbol(toToken)?.decimals || 9)).toFixed(6) : toToken;
            setLastSwapTxId(executeResult.signature || signature);
            setSwapSuccess(`Swapped ${fromAmount} ${fromToken} for ${outAmount} ${toToken}!`);
            setFromAmount('');
            setQuote(null);
            fetchWalletBalances();
          } else if (signature) {
        ...
        if (error.message && error.message.includes('rejected')) {
          ...
          setSwapError('Transaction failed. Try again.');
        }
      }
    } catch (error) {
      if (error.message && error.message.includes('rejected')) {
        ...
        setSwapError(error.message || 'Swap failed. Try again.');
      }
    } finally {
  ...
  const handleSetMaxAmount = () => {
    // If balances not loaded, fetch them first
    if (!balancesLoaded.current && connected && publicKey) {
      fetchWalletBalances(true);
    }
        ...
        <div className="swap-interface">
          ...
          {quote && connected && (
            <div className="quote-info">
              ...
              <div className="quote-row">
                ...
                <span className="quote-value">{settings.slippageTolerance}%</span>
              </div>
              <div className="quote-row">
                <span className="quote-label">MIN RECEIVE</span>
                <span className="quote-value">
                  {(parseFloat(outputAmount) * (1 - settings.slippageTolerance / 100)).toFixed(6)} {toToken}
                </span>
              </div>
            </div>
          )}
          ...
          {!settings.confirmTransactions && (
            <div className="swap-warning">
              <span>⚠</span> No confirm - executes instantly
            </div>
          )}
