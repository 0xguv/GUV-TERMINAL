...
const ArrowIcon = () => (
  <span className="transfer-arrow"><ArrowRight size={12} /></span>
);
...
const BridgeHistory = ({ walletAddress: propWalletAddress }) => {
  ...
  const getChainName = (chainId) => {
    ...
    // For unknown chains, show truncated ID
    const chainStr = String(chainId);
    if (chainStr.length > 6) {
      return `Chain ${chainStr.slice(0, 4)}...`;
    }
  ...
  const getTxUrl = (chainId, hash) => {
    ...
    if (!explorer) return null; // Return null instead of # for unsupported chains
            ...
            return (
              <div key={tx.id} className="bridge-history-item">
                ...
                <div className="history-tx-links">
                  {inHash && getTxUrl(inChain, inHash) && (
                    <a 
                  ...
                  {outHash && tx.status === 'success' && getTxUrl(outChain, outHash) && (
                    <a 
